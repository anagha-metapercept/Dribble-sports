...
if (!function_exists('posts_grid_shortcode')) {
	...
	function posts_grid_shortcode( $atts, $content = null, $shortcodename = '' ) {
		...
		$spans = $columns;
		$rand  = rand();
		$zoom_phrase = apply_filters( 'cherry_text_translate', of_get_option('portfolio_zoom'), 'portfolio_zoom' );
		...
		switch ($link) {
			...
			foreach ( $posts as $j => $post ) {
				...
				if (isset($zoom_phrase) && $custom_class == 'last-projects') {
					$zoom_icon  = '<span class="zoom-icon phrase"><div class="zoom"><em>'.$zoom_phrase.'</em></div><div class="helper"></div></span>';
				} else {
